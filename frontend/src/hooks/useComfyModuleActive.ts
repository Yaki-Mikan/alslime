...
// ComfyUI 連携モジュールの稼働状態を取り直す間隔。サイドカーは本体起動後に非同期で
// 起動し（ポート報告待ちの上限 15 秒）、異常終了で接続先が外れるため、一度きりの
// 取得では表示と実際の稼働（投入 API の受付条件）がずれる。
export const COMFY_MODULE_STATUS_POLL_INTERVAL_MS = 30_000;

/**
 * useComfyModuleActive は ComfyUI 連携の実体（サイドカー / in-process）の稼働状態を返す。
 *
 * ComfyUI 機能が有効な支援レベルの間だけ、初回・定期・画面の再表示時に取り直す。
 * 取得失敗は安全側の false とする。機能が無効になったとき・破棄時は確認を止める。
 * 返す setter は、画面からのモジュール導入・更新の結果を即時反映するために使う。
 */
export function useComfyModuleActive(backendUrl: string, featureEnabled: boolean) {
    ...
    useEffect(() => {
        ...
        const refresh = async () => {
            ...
            try {
                ...
                if (isCurrent()) setActive(modules.some(m => m.id === MODULE_COMFY && m.active));
            } catch {
        ...
        void refresh();
        const timer = setInterval(() => void refresh(), COMFY_MODULE_STATUS_POLL_INTERVAL_MS);
