...
import { saveEmotionPrompts, downloadEmotionPromptSample, type EmotionPrompts } from '../../api/characters';
...
export const EmotionPromptSection: React.FC<Props> = ({
    ...
    const entries = prompts.emotions[selectedEmotion] || [];
    ...
    useEffect(() => {
        ...
        const first = (prompts.emotions[selectedEmotion] || [])[0];
        if (first) {
    ...
    const handleSave = async () => {
        ...
        if (!trimmed) return;
        const others = entries.filter(e => e.title !== trimmed);
        const next: EmotionPrompts = {
            ...prompts,
            emotions: { ...prompts.emotions, [selectedEmotion]: [...others, { title: trimmed, prompt: expressionText.trim() }] },
        };
    ...
    const handleDelete = async () => {
        ...
        const remaining = entries.filter(e => e.title !== selectedTitle);
