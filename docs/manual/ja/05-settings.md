...
| ボタン | 内容 | 説明する章 |
| --- | --- | --- |
| 支援者機能 | 支援者ログインと状態確認 | [08](08-sponsor.md) |
| 設定パック | 設定のインポート / エクスポート | [07](07-settings-pack.md) |
| AIモデル設定 | 既定の会話プリセット・プロバイダ・モデル | 本章 2 |
| 画像生成設定 | ComfyUI 接続設定（**支援者機能が有効な場合のみ表示**） | [09](09-comfyui.md) |
...
| 同時実行数管理 | AI プロセスの同時実行数上限 | 本章 7 |
| デバッグ設定 | セッションのバックアップなど開発者向け | — |
| システム診断 | 環境の自己診断（読み取り専用） | 本章 8 |
...
「設定ファイルエディタ」の上部にある「設定自動生成」では、AIにキャラクター設定を作成してもらえます。使い方は、[06 設定自動生成](06-config-generation.md)をご覧ください。
...
環境の自己診断結果を表示する読み取り専用の画面です。動作がおかしいときは、まずここを確認してください（[11 困ったときは](11-troubleshooting.md)）。
...
画像付き応答を背景に表示する「セッション内背景画像」の設定は、**画像生成設定（支援者機能）の中**にあります。[09 ComfyUI 連携](09-comfyui.md) を参照してください。

---

前章: [04 ロールプレイ設定](04-roleplay.md) | 次章: [06 設定自動生成](06-config-generation.md)
