...
func TestClassify_分類テーブル(t *testing.T) {
	cases := []struct {
		...
		{"API共通基本指示はC分類", config.OpenAICompatSystemPromptFile("ja"), ClassConfig, "apiInstructions"},
		{"APIプリセット基本指示はC分類", config.OpenAICompatPresetPromptFile("openrouter", "en"), ClassConfig, "apiInstructions"},
		{"API接続別追加指示はC分類", config.OpenAICompatConnectionPromptFile("conn-abc123", "ja"), ClassConfig, "apiInstructions"},
		{"API接続先メタデータはC分類", config.APIProvidersFile, ClassConfig, "apiInstructions"},
		{"絶対除外: API秘密ストア", config.APIProviderSecretsFile, ClassForbidden, ""},
